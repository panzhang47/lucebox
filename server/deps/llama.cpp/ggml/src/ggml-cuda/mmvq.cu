...
static bool rocmfp2_fixed_k_enabled() {
    static const bool enabled = []() {
        const char * value = std::getenv("DFLASH_ROCMFP2_FIXED_K");
        return value && value[0] == '1' && value[1] == '\0';
    }();
    return enabled;
}

static bool rocmfp3_fixed_k_enabled() {
    static const bool enabled = []() {
...
static bool rocmfp4_unroll2_enabled() {
    static const bool enabled = []() {
        const char * value = std::getenv("DFLASH_ROCMFP4_UNROLL2");
        return value && value[0] == '1' && value[1] == '\0';
    }();
    return enabled;
}
...
static __global__ void mul_mat_vec_q(
    ...
    const uint32_t channel_x  = has_ids ? ids[channel_dst + ids_col*ids_stride] : fastdiv(channel_dst, channel_ratio);
    const uint32_t channel_y  = has_ids ? fastmodulo(channel_dst, nchannels_y) : channel_dst;

    uint32_t channel_xs[ncols_dst];
#pragma unroll
    for (int j = 0; j < ncols_dst; ++j) {
        channel_xs[j] = ids_multi_col ? ids[channel_dst + j*ids_stride] : channel_x;
    }
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    if constexpr (type == GGML_TYPE_Q3_0_ROCMFPX) {
        if (GGML_CUDA_CC_IS_RDNA3_5(cc) && ncols_dst == 1 &&
            ncols_x == 2048 && rocmfp3_fixed_k_enabled()) {
    ...
    if constexpr (type == GGML_TYPE_Q2_0_ROCMFP2) {
        if (GGML_CUDA_CC_IS_RDNA3_5(cc) && ncols_dst == 1 && rocmfp2_fixed_k_enabled()) {
            if (ncols_x == 4096) {
